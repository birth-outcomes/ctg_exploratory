...
% Run 12_fhrma_octave_preprocess.m before this script

clear;

% Get list of all files in training data and loop through them
train_path = 'fhrma/train_test_data/traindata_fhr';
...
for i = 1:length(train_files)
    ...
    name = erase(file, '.fhr')
    csvpath = ['fhrma/train_test_data/traindata_octave_csv/' name '.csv'];
    % Save to csv
    dlmwrite(csvpath, transpose(FHR), 'precision', '%.14f');
end

% Repeat for the test files
test_path = 'fhrma/train_test_data/testdata_fhr';
...
for i = 1:length(test_files)
    ...
    FHR = process(FHR1, FHR2);
    name = erase(file, '.fhr')
    csvpath = ['fhrma/train_test_data/testdata_octave_csv/' name '.csv'];
    dlmwrite(csvpath, transpose(FHR), 'precision', '%.14f');
end
